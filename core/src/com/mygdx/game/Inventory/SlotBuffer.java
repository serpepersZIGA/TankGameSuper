...
public class SlotBuffer {
    ...
    public void SlotPasteClient() {
        if(!MiddleMouse) {
            ...
            if(!(YCol < inventoryMain.HeightWindow & YCol > 0 & XCol<inventoryMain.WidthWindow &XCol> 0&
                ...
                //inventoryBuffer.SlotGenerationClient();
                EventDeleteItemClient event = new EventDeleteItemClient();
                event.x = ix;
                event.y = iy;
                event.conf = inventoryBuffer.InventoryType;
    ...
    public boolean SlotPasteClient(Slot Slot,int ix,int iy,InventoryInterface inventoryMain) {
...
//                }
                ...
                if (item2 != null) {
                    ...
                    event.item2 = inventoryBuffer.inventory.InventorySlots[this.ix][this.iy].ID;
                ...
                event.x2 = ix;
                event.y2 = iy;
                event.item1 = inventoryMain.inventory.InventorySlots[ix][iy].ID;
                event.InventoryType2 = inventoryBuffer.InventoryType;
                event.InventoryType = inventoryMain.InventoryType;
                for (int i = 0; i < UnitList.size(); i++) {
                    if (IDClient == UnitList.get(i).nConnect) {
                        event.i = i;
                        break;
            ...
            inventoryBuffer.SlotGeneration();
            return true;
        }
        return false;
    }
